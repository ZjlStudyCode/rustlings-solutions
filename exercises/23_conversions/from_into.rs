...
impl From<&str> for Person {
    fn from(s: &str) -> Self {
        ...
        let elements_count = params.len() as u8;
        let name = params.first().unwrap();
        let age: Result<u8, _> = params.last().unwrap().parse();
        if elements_count != 2 || name.is_empty() || age.is_err() {
            Self::default()
        } else {
            Self {
                name: name.to_string(),
